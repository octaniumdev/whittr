...
const opening = [
    ...
    "is here to fight!", "is here to chew gum and fight crime!",
    "has stolen your pizza!", "just joined the chat, glhf!",
    "just joined, everyone pretend you're busy!",
    "joined your party.", "We have been expecting you ( ͡° ͜ʖ ͡°)",
    "has brought pizza!", "please leave your weapons by the door.",
    "has appeared.", "just slid into the chat ( ͡° ͜ʖ ͡°)",
    "has just landed.", "needs to be nerfed", "is here to slide into your DMs"
];
...
io.on("connection", (socket) => {
    ...
    const id = crypto.createHmac("sha512", secret).update(socket.id).digest("hex");
        ...
        if (!isRealString(req.name) || !isRealString(req.room)) {
            ...
            if (req.name.length > 16) {
                ...
                if (req.room.length > 8) {
                    ...
                    const room = crypto.createHmac("sha512", secret).update(req.room).digest("hex");
                    socket.join(room);
    ...
    socket.on("disconnect", () => {
        ...
        if (user) {
            // 
            io.to(user.room).emit("updateUserList", onlineUsers.getUserList(user.room));
